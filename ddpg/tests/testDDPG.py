...
class TestBuildActorModel(unittest.TestCase):
    ...
    #     graph = model.graph
    #     states_ = graph.get_collection_ref("states_")
    #     action_ = graph.get_collection_ref("action_")
    #
    #
    #
    ...
    #     evalDict = model.run(fetches, feed_dict={state_: stateBatch, groundTruthAction_: actionBatch,
    #                                              groundTruthValue_: valueBatch})
    ...
    def testActByTrainModelWithEmptyInputLayerWidths(self, states, miniBatchSize):
        ...
        self.assertAlmostEqual(float(actionAngleTrainModelOutput), float(actionAngleGroundTruth), places=5)
