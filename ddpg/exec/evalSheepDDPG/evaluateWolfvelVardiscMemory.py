...
class EvaluateNoiseAndMemorySize:
    ...
    def __call__(self, df):
        ...
        wolfSpeed = df.index.get_level_values('wolfSpeed')[0]
        print('-------------------------------------------------------------------')
        print('varianceDiscount: ', varianceDiscount, ', bufferSize: ', bufferSize, ', wolfSpeed: ', wolfSpeed)
...
def main():
    ...
    independentVariables['wolfSpeed'] = [1, 2, 3]
    ...
    nRows = len(independentVariables['varianceDiscount'])
    numplot = 1
    axs = []
    figure = plt.figure(dpi= 200)
    figure.set_size_inches(7.5, 6)
    figure.suptitle('buffer: ' + str(independentVariables['bufferSize'])+
                    ', varDiscount: '+ str(independentVariables['varianceDiscount']) +
                    ', wolfSpeed: '+ str(independentVariables['wolfSpeed']), fontsize = 8)

    for varianceDiscount, outterSubDf in resultDF.groupby('varianceDiscount'):
        for bufferSize, innerSubDf in outterSubDf.groupby('bufferSize'):
            ...
            subplot.set_title('varianceDiscount = ' + str(varianceDiscount) + ' bufferSize = ' + str(bufferSize), fontsize=5)
