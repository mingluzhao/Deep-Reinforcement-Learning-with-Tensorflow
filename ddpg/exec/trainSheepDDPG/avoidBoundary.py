...
from environment.chasingEnv.envNoPhysics import Reset, TransitForNoPhysics, getIntendedNextState, \
    StayWithinBoundary, TransitWithSingleWolf, GetAgentPosFromState, IsBoundaryTerminal
import numpy as np
...
def main():
    ...
    rewardSheep = RewardFunctionCompete(sheepAliveBonus, sheepTerminalPenalty, isBoundaryTerminal)
    actionCostRate = 1
    ...
    runEpisode = RunEpisode(reset, runDDPGTimeStep, maxTimeStep, isBoundaryTerminal)
    ...
    parameters = {'wolfSpeed':0, 'dimension': actionDim, 'maxEpisode': maxEpisode, 'maxTimeStep': maxTimeStep, 'minibatchSize': minibatchSize, 'gamma': gamma,
                                 'learningRateActor': learningRateActor, 'learningRateCritic': learningRateCritic}

    modelSaveDirectory = "../trainedDDPGModels/noWolfAvoidBoundary"
