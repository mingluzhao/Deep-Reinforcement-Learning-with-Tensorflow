...
#     def __init__(self, actorModel, actByTrain):
#         self.actorModel = actorModel
#         self.actByTrain = actByTrain
#
#     def __call__(self, agentObs, runTimeStep):
#         policyTrainOutput = self.actByTrain(self.actorModel, agentObs)
#         sampleNoise = tf.random_uniform(tf.shape(policyTrainOutput))
#         actionSample = U.softmax(policyTrainOutput - tf.log(-tf.log(sampleNoise)), axis=-1)
...
class ActOneStepMADDPGWithNoise:
    ...
    def __call__(self, agentObs, runTimeStep = None):
        agentObs = np.array([[ 0. ,         0. ,        -0.72976433, -0.46009255,  0.61463679,  0.06004687, 0. ,         0.        ]])
        actorGraph = self.actorModel.graph
        states_ = actorGraph.get_collection_ref("states_")[0]
        policyTrainOutput = actorGraph.get_collection_ref("trainAction_")[0]

        sampleNoise = tf.random_uniform(tf.shape(policyTrainOutput), seed = 1)
        actionSample = U.softmax(policyTrainOutput - tf.log(-tf.log(sampleNoise)), axis=-1)
        Action = self.actorModel.run(actionSample, feed_dict={states_: agentObs})
        action = Action[0]

# [[ 0.          0.         -0.72976433 -0.46009255  0.61463679  0.06004687 0.          0.        ]]
# action = [0.00883901 0.8419195  0.13447414 0.00757123 0.00719616]
        return action
    ...
    def __call__(self, layersWidths, summaryPath="./tbdata"):
        ...
        with graph.as_default():
            with tf.name_scope("inputs"):
                ...
                actionGradients_ = tf.placeholder(tf.float32, [None, self.actionDim], name='actionGradients_')
            ...
            with tf.variable_scope("trainHidden"):
                ...
                for i in range(len(layersWidths)):
                    activation_ = layers.fully_connected(activation_, num_outputs= layersWidths[i], activation_fn=tf.nn.relu, scope="fc{}".format(i+1), weights_initializer=tf.initializers.glorot_uniform(seed=0))

                trainActivationOutput_ = layers.fully_connected(activation_, num_outputs= self.actionDim, activation_fn= None, scope="fc{}".format(len(layersWidths)+1), weights_initializer=tf.initializers.glorot_uniform(seed=0))
            ...
            with tf.variable_scope("targetHidden"):
                ...
                targetActivationOutput_ = layers.fully_connected(activation_, num_outputs= self.actionDim, activation_fn=None, scope="fc{}".format(len(layersWidths)+1), weights_initializer=tf.initializers.glorot_uniform(seed=0))
    ...
    def __call__(self, layersWidths , summaryPath="./tbdata"):
        ...
        with graph.as_default():
            ...
            with tf.variable_scope("trainHidden"):
                ...
                trainValues_ = layers.fully_connected(activation_, num_outputs= 1, activation_fn= None, scope="fc{}".format(len(layersWidths)+1), weights_initializer=tf.initializers.glorot_uniform(seed=0))
            ...
            with tf.variable_scope("targetHidden"):
                ...
                targetValues_ = layers.fully_connected(activation_, num_outputs= 1, activation_fn= None, scope="fc{}".format(len(layersWidths)+1), weights_initializer=tf.initializers.glorot_uniform(seed=0))
...
#
#                 trainActivationOutput_ = layers.fully_connected(activation_, num_outputs= self.actionDim, activation_fn= None, scope="fc{}".format(len(layersWidths)+1))
#
#             with tf.variable_scope("targetHidden"):
#                 activation_ = states_
#                 for i in range(len(layersWidths)):
#                     activation_ = layers.fully_connected(activation_, num_outputs= layersWidths[i], activation_fn=tf.nn.relu, scope="fc{}".format(i+1))
#
#                 targetActivationOutput_ = layers.fully_connected(activation_, num_outputs= self.actionDim, activation_fn=None, scope="fc{}".format(len(layersWidths)+1))
#
#             with tf.name_scope("updateParameters"):
#                 trainParams_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='trainHidden')
#                 targetParams_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='targetHidden')
...
#                     activation_ = layers.fully_connected(activation_, num_outputs= layersWidths[i], activation_fn=tf.nn.relu, scope="fc{}".format(i+1))
#
#                 trainValues_ = layers.fully_connected(activation_, num_outputs= 1, activation_fn= None, scope="fc{}".format(len(layersWidths)+1))
#
#             with tf.variable_scope("targetHidden"):
#                 activation_ = tf.concat([states_, actionTarget_], axis=1)
#                 for i in range(len(layersWidths)):
#                     activation_ = layers.fully_connected(activation_, num_outputs= layersWidths[i], activation_fn=tf.nn.relu, scope="fc{}".format(i+1))
#
#                 targetValues_ = layers.fully_connected(activation_, num_outputs= 1, activation_fn= None, scope="fc{}".format(len(layersWidths)+1))
#
#             with tf.name_scope("parameters"):
#                 trainParams_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='trainHidden')
#                 targetParams_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='targetHidden')
