...
def unscale_action(actionLow, actionHigh, scaled_action):
    return actionLow + (0.5 * (scaled_action + 1.0) * (actionHigh - actionLow))
    ...
    def __init__(self, buildPolicyNet, numStateSpace, actionDim, session, hyperparamDict, actionRange, agentID = None):
        ...
        with tf.variable_scope(self.scope):
            ...
            with tf.variable_scope("actionOutput"):
                ...
                self.action_ = self.mu_ + tf.random_normal(tf.shape(self.mu_)) * sigma_
                self.logPi_ = gaussian_likelihood(self.action_, self.mu_, self.logSigma_)

                deterministic_policy, self.action_, self.logPi_ = apply_squashing_func(self.mu_, self.action_, self.logPi_)

                self.scaledAction_ = unscale_action(self.actionLow, self.actionHigh, self.action_)
...
class TrainSoftACOneStep:
    ...
    def __call__(self, miniBatch):
        ...
        nextLogPi = self.policyNet.getLogPi(nextStateBatch).reshape(-1, 1)
