        ...
        for episodeID in range(self.maxEpisode):
            ...
            if self.multiAgent:
                ...
                if episodeID % self.printEpsFrequency == 0:
                    ...
                    print("episodes: {}, last {} eps mean episode reward: {}, agent mean reward: {}, time: {}".format(
                        episodeID, self.printEpsFrequency, lastTimeSpanMeanReward,
                        [np.mean(rew[-self.printEpsFrequency:]) for rew in agentsEpsRewardList], round(time.time()-tStart, 3)))
                    tStart = time.time()
