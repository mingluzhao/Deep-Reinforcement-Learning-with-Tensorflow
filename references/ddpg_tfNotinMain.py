...
class DDPG:
    def __init__(self, initializeMemory, sampleFromMemory, reset, addActionNoise):
    ...
    def __call__(self, actorModel, actorTargetModel, criticModel, criticTargetModel, trainActor, trainCritic):
        ...
        for episode in range(maxEpisode):
            ...
            for timeStep in range(timeStepRange):
                ...
                stateBatch, actionBatch, rewardBatch, nextStateBatch = self.sampleFromMemory(replayBuffer,
                                                                                             minibatchSize)

                criticLoss, criticModel, criticTargetModel = trainCritic(actorTargetModel, criticTargetModel,
                                                                         criticModel,
                                                                         nextStateBatch, rewardBatch, stateBatch,
                                                                         actionBatch)
...
def main():
    ...
    with actorGraph.as_default():
        with tf.name_scope("inputs"):
            ...
            actionGradients_ = tf.placeholder(tf.float32, [None, numActionSpace])
        ...
        with tf.name_scope("trainingParams"):
            ...
            initBias_ = tf.constant_initializer(0.01)
            # learningRate_ = tf.constant(0, dtype=tf.float32)
            # numActorFC1Unit_ = numActorFC1Unit
            # numActionSpace_ = numActionSpace
            #
            # tf.add_to_collection("initWeight", initWeight_)
            # tf.add_to_collection("initBias", initBias_)
            # tf.add_to_collection("learningRate", learningRate_)
        ...
        with tf.name_scope("hidden"):
            fullyConnected1_ = tf.layers.dense(inputs=state_, units=numActorFC1Unit, activation=tf.nn.relu,
                                               kernel_initializer=initWeight_, bias_initializer=initBias_,
                                               name='fullyConnected1_', trainable=trainable)
            actionsOutput_ = tf.layers.dense(inputs=fullyConnected1_, units=numActionSpace, activation=tf.nn.tanh,
                                             kernel_initializer=initWeight_, bias_initializer=initBias_,
                                             name='actions', trainable=trainable)
            tf.add_to_collection("actionsOutput_", actionsOutput_)
        ...
        with tf.name_scope("train"):
            optimizer = tf.train.AdamOptimizer(learningRateActor)  # (-self.lr)  # (- learning rate) for ascent policy
            trainOpt_ = optimizer.apply_gradients(zip(policyGradient_, actorParams_))
    ...
    with actorTargetGraph.as_default():
        ...
        with tf.name_scope("hidden"):
            ...
            tf.add_to_collection("replaceParam_", replaceParam_)
    actorTargetInit = tf.global_variables_initializer()
    actorTargetWriter = tf.summary.FileWriter('tensorBoard/actorTargetOnlineDDPG', graph=actorTargetGraph)
    actorTargetModel = tf.Session(graph=actorTargetGraph)
    actorTargetModel.run(actorTargetInit)
    ...
    with criticGraph.as_default():
        ...
        with tf.variable_scope('hidden'):
            w1_s = tf.get_variable('w1_s', [numStateSpace, numActorFC1Unit], initializer=initWeight_,
                                   trainable=trainable)
            w1_a = tf.get_variable('w1_a', [numActionSpace, numActorFC1Unit], initializer=initWeight_,
                                   trainable=trainable)
            b1 = tf.get_variable('b1', [1, numActorFC1Unit], initializer=initBias_, trainable=trainable)
            layer1 = tf.nn.relu(tf.matmul(state_, w1_s) + tf.matmul(action_, w1_a) + b1)
            qValue_ = tf.layers.dense(inputs=layer1, units=1, activation=None, name='value',
                                      kernel_initializer=initWeight_, bias_initializer=initBias_,
                                      trainable=trainable)  # Q(s,a)
    ...
    with criticTargetGraph.as_default():
        ...
        with tf.variable_scope('hidden'):
            ...
            criticTargetParams_ = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,
                                                    scope='hidden')  # [trainable network]
