...
class ResetMultiAgentChasing:
    def __init__(self, world, numTotalAgents):
        # self.world = world
    ...
    def __call__(self):
        ...
        for agent in self.world.agents:
            agent.state.p_pos = np.random.uniform(-1, +1, self.positionDimension)
            agent.state.p_vel = np.zeros(self.positionDimension)
            agent.state.c = np.zeros(self.world.dim_c)
        for i, landmark in enumerate(self.world.landmarks):
            if not landmark.boundary:
                landmark.state.p_pos = np.random.uniform(-0.9, +0.9, self.positionDimension)
                landmark.state.p_vel = np.zeros(self.positionDimension)
