...
class BuildMADDPGModels:
    def __init__(self, actionDim, actionDimList, obsShapeList, actionRange = 1):
    ...
    def __call__(self, layersWidths, agentID):
        ...
        with graph.as_default():
            with tf.variable_scope("inputs/"+ agentStr):
                ...
                allAgentsActions_ = [tf.placeholder(dtype=tf.float32, shape=[None, agentActionDim], name="action"+str(i)) for i, agentActionDim in enumerate(self.actionDimList)]
                allAgentsNextActionsByTargetNet_ = [tf.placeholder(dtype=tf.float32, shape=[None, agentActionDim], name= "actionTarget"+str(i)) for i, agentActionDim in enumerate(self.actionDimList)]
...
class BuildMADDPGModelsForCentralControl:
    def __init__(self, actionDimList, numAgentsToControl, obsShapeList, actionRange = 1):
        self.actionDimList = actionDimList
        self.centralActionDim = actionDimList[0]
        self.singleActionDim = actionDimList[1]
    ...
    def __call__(self, layersWidths, agentID):
        ...
        with graph.as_default():
            ...
            with tf.variable_scope("actor/trainHidden/"+ agentStr): # act by personal observation
                ...
                for i in range(len(layersWidths)):
                    actorTrainActivation_ = layers.fully_connected(actorTrainActivation_, num_outputs= layersWidths[i], activation_fn=tf.nn.relu)

                actorTrainActivation_ = layers.fully_connected(actorTrainActivation_, num_outputs= self.centralActionDim, activation_fn= None)
            ...
            with tf.variable_scope("actor/targetHidden/"+ agentStr):
                ...
                for i in range(len(layersWidths)):
                    actorTargetActivation_ = layers.fully_connected(actorTargetActivation_, num_outputs= layersWidths[i],  activation_fn=tf.nn.relu)

                actorTargetActivation_ = layers.fully_connected(actorTargetActivation_, num_outputs= self.centralActionDim, activation_fn=None)
