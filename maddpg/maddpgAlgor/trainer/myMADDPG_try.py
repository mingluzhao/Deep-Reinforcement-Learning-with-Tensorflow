    ...
    def __call__(self, layersWidths, agentID):
        ...
        with graph.as_default():
            ...
            with tf.variable_scope("actor/trainHidden/"+ agentStr): # act by personal observation
                ...
                actorTrainActivation_ = layers.fully_connected(actorTrainActivation_, num_outputs= self.actionDim, activation_fn= None)
            ...
            with tf.variable_scope("actor/targetHidden/"+ agentStr):
                ...
                actorTargetActivation_ = layers.fully_connected(actorTargetActivation_, num_outputs= self.actionDim, activation_fn=None)
            ...
            with tf.variable_scope("critic/trainHidden/" + agentStr, reuse= True):
                ...
                reSampleNoiseTrain_ = tf.random_uniform(tf.shape(trainAction_))
                noisyActionResampled_ = U.softmax(trainAction_ - tf.log(-tf.log(reSampleNoiseTrain_)), axis=-1) # give this to q input

                criticInputActionList[agentID] = noisyActionResampled_
