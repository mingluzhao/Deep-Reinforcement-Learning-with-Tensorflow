...
def main():
    ...
    rewardWolfWithCost = lambda state, action, nextState: np.array(rewardWolf(state, action, nextState)) - np.array([getActionCost(action[wolfID]) for wolfID in wolvesID])

    punishForOutOfBound = PunishForOutOfBound()
    rewardSheep = RewardSheep(wolvesID, sheepsID, entitiesSizeList, getPosFromAgentState, isCollision,
                              punishForOutOfBound)
    rewardSheepWithCost = lambda state, action, nextState: np.array(rewardSheep(state, action, nextState)) - np.array([getActionCost(action[sheepID]) for sheepID in sheepsID])


    rewardFunc = lambda state, action, nextState: \
        list(rewardWolfWithCost(state, action, nextState)) + list(rewardSheepWithCost(state, action, nextState))
