...
def main():
    file = open('conditionsMADDPGWithProbKill.json', 'r')
    ...
    startTime = time.time()
    fileName = 'runMyMADDPGchasingWithProbKillSensToDist.py'
    numSample = None
    numCpuToUse = int(0.8 * os.cpu_count())
    excuteCodeParallel = ExcuteCodeOnConditionsParallel(fileName, numSample, numCpuToUse)
    print("start")

    conditionLevels = [(numWolves, sheepSpeedMultiplier, costActionRatio, rewardSensitivityToDistance, biteReward)
    ...
    for condition in conditionLevels:
        numWolves, sheepSpeedMultiplier, costActionRatio, rewardSensitivityToDistance, biteReward = condition
        parameters = {'numWolves': numWolves, 'sheepSpeedMultiplier': sheepSpeedMultiplier, 'costActionRatio': costActionRatio,
                      'rewardSensitivityToDistance': rewardSensitivityToDistance, 'biteReward': biteReward}
