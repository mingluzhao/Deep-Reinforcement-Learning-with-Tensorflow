...
def main():
    startTime = time.time()
    fileName = 'iterRunMyMADDPGchasing.py'
    numSample = None
    ...
    fileIDList = list(range(30))

    conditionLevels = [(wolfNum, sheepNum, blockNum, timeStep, sheepSpeed, individReward, costRatio, fileID)
    ...
    for condition in conditionLevels:
        numWolves, numSheeps, numBlocks, maxTimeStep, sheepSpeedMultiplier, individualRewardWolf, costActionRatio, fileID = condition
        parameters = {'numWolves': numWolves, 'numSheeps': numSheeps, 'numBlocks': numBlocks,
                      ...
                      'individualRewardWolf': individualRewardWolf, 'costActionRatio': costActionRatio, 'fileID': fileID}
