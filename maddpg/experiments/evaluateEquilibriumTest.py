...
class EvaluateWolfSheepTrain:
    ...
    def __call__(self, df):
        ...
        recoveredSelfishIndex = df.index.get_level_values('recoveredSelfishIndex')[0]
        costActionRatio = df.index.get_level_values('costActionRatio')[0]
        fileID = float(df.index.get_level_values('fileID')[0])
        epsID = int(df.index.get_level_values('epsID')[0])
        continueSelfishIndex = 10000.0 if fileID < 10 else 0.0
        # recoveredSelfishIndex, continueSelfishIndex = trainingSequence
...
class GetSheepModelPaths:
    def __init__(self, sheepSpeedList, costActionRatioList, recoveredSelfishIndexList):
        self.sheepSpeedList = sheepSpeedList
        self.costActionRatioList = costActionRatioList
        self.recoveredSelfishIndexList = recoveredSelfishIndexList

    def __call__(self, numWolves, numSheeps, numBlocks, maxEpisode, maxTimeStep):
        dirName = os.path.dirname(__file__)
        biteReward = 0.0
        killProportion = 0.2
        continueSelfishIndexFileIDList = [(10000.0, fileID) for fileID in range(10)] + [(0.0, fileID) for fileID in range(10, 20)]
        fileNameList = ["maddpg{}wolves{}sheep{}blocks{}episodes{}stepSheepSpeed{}WolfActCost{}biteReward{}killPercent{}initSensitive{}later{}file{}_agent{}60000eps".format(
            numWolves, numSheeps, numBlocks, maxEpisode, maxTimeStep, sheepSpeedMultiplier, costActionRatio,
            biteReward, killProportion, recoveredSelfishIndex, continueSelfishIndex, fileID, numWolves)
            for sheepSpeedMultiplier in self.sheepSpeedList
            for recoveredSelfishIndex in self.recoveredSelfishIndexList
            for continueSelfishIndex, fileID in continueSelfishIndexFileIDList
...
def main():
    ...
    sheepSpeedList = [0.75]
    costActionRatioList = independentVariables['costActionRatio']
    recoveredSelfishIndexList = independentVariables['recoveredSelfishIndex']
    getSheepModelPaths = GetSheepModelPaths(sheepSpeedList, costActionRatioList,recoveredSelfishIndexList)
    ...
    resultLoc = os.path.join(resultPath, 'evalEquiWithKill.pkl')
    saveToPickle(resultDF, resultLoc)

    print('saved to evalEquiWithKill.pkl')
