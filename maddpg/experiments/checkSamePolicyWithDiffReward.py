...
1. find useful states and generate the action trajectories starting from the same state
2. If different
    1. sample actions with the same magnitude
    2. evaluate actions using critics of individual vs shared rewards
...
def main():
    ...
    costActionRatio = 0.0
    rewardSensitivityToDistance = 0.0#10000.0
    biteReward = 0.0
    ...
    # reset = ResetMultiAgentChasing(numAgents, numBlocks)
    state = [[-0.3523,  0.3504, -0.3566, -0.5418], [-0.3358,  0.1583, -0.199,  -0.9106],
                     [-0.5682, -0.0665, -0.9999,  0.0165], [-0.7892,  0.1301, -0.5169, -1.1928], [-0.5682, -0.3681,  0. ,     0.    ],
                     [ 0.7602, -0.537,   0.,      0.    ]]

    reset = lambda: state

    actions = [[3.200e-03, 1.000e-04, 9.902e-01, 5.000e-04, 6.000e-03],
