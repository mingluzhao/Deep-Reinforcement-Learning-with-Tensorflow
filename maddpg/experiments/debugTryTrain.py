...
ddpg = False
policyPath = os.path.join(dirName, '..', 'policy1WolfMADDPG1SheepMADDPGWithTryTrain1')
...
def main():
    ...
    with U.single_threaded_session():
        ...
        # obs_n = observe(state)
        obs_n = env.reset()
        state = [list(env.agents[i].state.p_pos) + list(env.agents[i].state.p_vel) for i in range(numAgents)]
        ...
        while True:
            ...
            new_obs_n1, rew_n1, done_n1, info_n1 = env.step(action_n)
            print('------------------------------------------')
            print(obs_n)
            print(rew_n1)
            print(new_obs_n1)
