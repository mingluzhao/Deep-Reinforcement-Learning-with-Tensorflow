...
def main():
    ...
    policyPath = os.path.join(dirName, '..', 'trainedModels', 'sourceCodeModelsWithRefEnv_3catchrew', 'sourceCodeModelsWithRefEnv_3catchrew1111', fileName)
    print(policyPath)
    ...
    fileName = "maddpgSourceCode{}wolves{}sheep{}blocksfile{}_agent".format(numWolves, numSheeps, numBlocks,fileID)
    sheepPath = os.path.join(dirName, '..', 'trainedModels', 'sourceCodeModelsWithRefEnv', 'newEnv_punishment30', fileName)

    with U.single_threaded_session():
        U.initialize()

        U.load_state(policyPath)
        sheepTrainer = trainers[-1]

        U.load_state(sheepPath)
        wolvesTrainers = trainers[:-1]

        trainers = wolvesTrainers + [sheepTrainer]
        policy = lambda state: [agent.action(obs) for agent, obs in zip(trainers, observe(state))]
