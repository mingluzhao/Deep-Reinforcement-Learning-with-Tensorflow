...
def evaluateWolfSheepTrain(df):
    ...
    modelPaths = [os.path.join(dirName, '..', 'trainedModels', 'evalOneSheep_2575steps_1to6wolves_11.25speed', fileName + str(i)) for i in
                  range(numAgents)]
...
def main():
    ...
    independentVariables['sheepSpeedMultiplier'] = [1, 1.25]
    ...
    for keyCol, outterSubDf in resultDF.groupby('sheepSpeedMultiplier'):
        ...
        plotCounter += 1
        axForDraw.set_aspect(0.015, adjustable='box')

        plt.legend(title='Wolf type')

    plt.suptitle('1~6 wolves vs 1 sheep, train 75 steps, sampling 75 steps/eps')


    plt.savefig(os.path.join(resultPath, 'evalWolfNumberWithOneSheep_75steps_sample75'))
    plt.show()
