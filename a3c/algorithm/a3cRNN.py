...
class GlobalNet(object):
    def __init__(self, stateDim, actionDim, hyperparamDict):
        ...
        self.cellSize = hyperparamDict['cellSize']

        with tf.variable_scope(self.scope):
            with tf.variable_scope("inputs"):
                self.states_ = tf.placeholder(tf.float32, [None, self.stateDim], name='states_')

            with tf.variable_scope('criticNet'):  # only critic controls the rnn update
                statesExpanded_ = tf.expand_dims(self.states_, axis=1, name='timely_input')  # [time_step, feature] => [time_step, batch, feature]  # should be [batch, feature] -> [batch, timestep, feature]
                rnnCell = tf.contrib.rnn.BasicRNNCell(self.cellSize)
                self.rnnInitState = rnnCell.zero_state(batch_size=1, dtype=tf.float32)  # shape = [1, 128]
                rnnOutputs, self.rnnFinalState = tf.nn.dynamic_rnn(cell=rnnCell, inputs=statesExpanded_, initial_state=self.rnnInitState, time_major=True)
            ...
            with tf.variable_scope('actorNet'):  # state representation is based on critic
                rnnCellOutput = tf.stop_gradient(rnnCellOutput, name='criticRNNoutput')  # from what critic think it is
                actorTrainActivation_ = rnnCellOutput
...
class WorkerNet(object):
    def __init__(self, stateDim, actionDim, hyperparamDict, actionRange, scope, globalModel, session):
        ...
        with tf.variable_scope(self.scope):
            ...
            with tf.variable_scope('criticNet'):  # only critic controls the rnn update
                statesExpanded_ = tf.expand_dims(self.states_, axis=1, name='timely_input')  # [time_step, feature] => [time_step, batch, feature]  ** here timestep =128, batchsize = 1
                rnnCell = tf.contrib.rnn.BasicRNNCell(self.cellSize)
                self.rnnInitState_ = rnnCell.zero_state(batch_size=1, dtype=tf.float32)  # shape = [1, 128]
                rnnOutputs_, self.rnnFinalState_ = tf.nn.dynamic_rnn(cell=rnnCell, inputs=statesExpanded_, initial_state=self.rnnInitState_, time_major=True)
            ...
            with tf.variable_scope('actorNet'):  # state representation is based on critic
                actorTrainActivation_ = tf.stop_gradient(rnnCellOutput_, name='criticRNNoutput')  # from what critic think it is
                for numUnits in self.actorLayersWidths:
                    actorTrainActivation_ = tf.layers.dense(actorTrainActivation_, numUnits, self.actorActivFunction, kernel_initializer=self.weightInit)
                mu_ = tf.layers.dense(actorTrainActivation_, self.actionDim, self.actorMuOutputActiv, kernel_initializer=self.weightInit, name='mu_')
                sigma_ = tf.layers.dense(actorTrainActivation_, self.actionDim, self.actorSigmaOutputActiv, kernel_initializer=self.weightInit, name='sigma_')
            ...
            with tf.variable_scope('loss'):
                ...
                logProb = normal_dist.log_prob(self.action_)
                actionVal_ = logProb * tf.stop_gradient(tdError_)
            ...
            with tf.variable_scope("trainOpt"):
                self.actorOpt_ = tf.train.RMSPropOptimizer(self.actorLR, name=self.scope + '/RMSPropOptActor', decay = 0.95)
                self.criticOpt_ = tf.train.RMSPropOptimizer(self.criticLR, name= self.scope + '/RMSPropOptCritic', decay = 0.95)
    ...
    def initializeRnnState(self):
        rnnInitState = self.session.run(self.rnnInitState_)
        return rnnInitState

    def act(self, state, rnnInitState):
        state = state[np.newaxis, :]
        action, rnnFinalState = self.session.run([self.actionOutput_, self.rnnFinalState_], feed_dict={self.states_: state, self.rnnInitState_: rnnInitState})
        return action, rnnFinalState

    def pullFromGlobalNet(self):
        self.session.run([self.pullActorParams_, self.pullCriticParams_])

    def pushToGlobalNet(self, state, action, valueTarget, rnnKeptInitState):
        self.session.run([self.updateActor_, self.updateCritic_], feed_dict= {self.states_: state, self.action_: action, self.valueTarget_: valueTarget, self.rnnInitState_: rnnKeptInitState})

    def getBootStrapValue(self, state, rnnInitState):
        state = state.reshape(1, -1)
        value = self.session.run(self.value_, feed_dict={self.states_: state, self.rnnInitState_: rnnInitState})
        return value
...
class A3CWorker:
    ...
    def work(self):
        ...
        while not self.coord.should_stop() and self.globalCount.count < self.globalMaxT:
            state = self.reset()
            epsReward = 0
            rnnInitState = self.workerNet.initializeRnnState()
            rnnKeptInitState = rnnInitState.copy() ###
            ...
            for timeStep in range(self.maxTimeStepPerEps):
                ...
                if self.workerTotalSteps % self.updateInterval == 0 or done:
                    ...
                    self.workerNet.pushToGlobalNet(stateBatch, actionBatch, valueTargetBatch, rnnKeptInitState)
                    ...
                    rnnKeptInitState = rnnFinalState.copy()
